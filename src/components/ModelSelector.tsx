...
import { Check, Cpu, ChevronUp } from 'lucide-react'
...
export function ModelSelector({ currentModel, onModelChange }: ModelSelectorProps) {
  ...
  // 显示当前模型名称（缩短长名称）
  const displayModel = currentModel || allModels[0]
  const shortName = displayModel

  // 找到当前模型所属的供应商颜色和图标
  const currentProvider = modelsByProvider.find(p => p.models.includes(displayModel))
